...
{
	if(strlen(s) == 0)
		sysfatal("empty ignore argument");
	if((ignorerx = regcomp(s)) == nil)
		sysfatal("invalid ignore regexp: %r");
	if((fullpath = s_new()) == nil)
		sysfatal("s_new: %r");
}
...
ignored(char *s)
{
	return ignorerx != nil && regexec(ignorerx, s, nil, 0);
}
...
hasgit(char *dir)
{
	...
	s_append(gitdir, dir);
	s_append(gitdir, "/.git");
	int status= dirstat(s_to_c(gitdir)) != nil;
	return status;
}
...
dofile(char *path, Dir *f, int pathonly)
{
	...
	if(ignorerx != nil){
		s_reset(fullpath);
		s_append(fullpath, path);
		s_putc(fullpath, '/');
		s_append(fullpath, f->name);
		if(ignored(s_to_c(fullpath)))
			return;
	}
...
walk(char *path, Dir *cf, long depth)
{
	...
	if(ignored(path) || (depth > 1 && hasgit(path)))
		return;
	if(depth >= maxdepth)
		goto nodescend;
...
nodescend:
	depth--;
	if(dflag && depth >= mindepth && !ignored(path))
		dofile(path, cf, 0);
}
...
main(int argc, char **argv)
{
	...
	case 'I': /* ignore dirs containing .git/ */
		Iflag++;
		if((gitdir = s_new()) == nil)
			sysfatal("s_new: %r");
		break;

	case 'i': ignore(EARGF(usage())); break;
